 ...
 *   2026-09-01  Jimmy/Claude  validateRouteSteps 擋下「簽到 X 為止」＋「跳過 X」的矛盾設定
 *                              （實際跑起來 X 那一關會被跳過、等於沒簽，但畫面上看不出來）
 */
(() => {
  'use strict';
  ...
  const validateRouteSteps = (rows, known) => {
    ...
    rows.forEach((row, idx) => {
      ...
      if (seg === SEG.EMPLOYEE_CHAIN) {
        ...
        // 「簽到 X 為止」＋「跳過 X」是自相矛盾的設定：引擎走到 X 就停，但 X 在跳過
        // 清單裡所以不列入簽核——結果等同「簽到 X 的前一關為止」，卻讓人看不出來。
        // 真要那個效果，直接把截止職稱設成前一關即可。
        if (stopAt && skip.includes(stopAt)) {
          errs.push(
            `第 ${n} 列：「簽到職稱為止」與「跳過的職稱」都指定了「${stopAt}」，` +
            `這樣那一關會被跳過、等於沒簽。請從「跳過的職稱」移除它。`
          );
        }
