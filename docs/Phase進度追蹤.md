...
- **測試**:`npm test` → **268 項全過**（9 檔）

### 2026-09-01 新增：人事異動對照（tools/11）＋ 工具按鈕盤點

Jimmy 的三個需求：①職稱與後台不符要能快速查改、②單位變更同理、③把按鈕統整分析。

**做了什麼**

- `core/08-directory.js`（新）— 後台使用者／組織／職務的存取層 + `unit_name`／`title_level`
  對應規則。原本埋在 `tools/05`，抽出來由 05 與 11 共用；新增 **identityMap** 保留
  「單位×職務」的配對（05 原本的 orgMap／titleMap 各自攤平，兼任者會對不回同一個身分）。
- `tools/11-org-sync-check.js`（新，1,098 行）— 「人事異動對照」按鈕，掛 685／686。
- `tests/org-sync-check.test.js`（新）— 30 項，覆蓋比對、分類、寫入計畫。
- `docs/10-工具按鈕盤點與分組.md`（新）— 11 顆按鈕的盤點、分組建議、**修正過的上傳順序表**。

**分類邏輯（為什麼要分辨成因）**

同一個「後台對不上」的現象，成因不同、改法完全相反：

| 判定 | 條件 | 改法 |
|------|------|------|
| 單位改名 | 同一 `unit_name` 底下**全部**人一致指向同一個新單位（≥2 筆） | 就地改 `unit_name` |
| 關卡改名 | 同一 `role_name` **全部**人一致指向同一組新單位職稱（≥2 筆） | 就地改兩個欄位 |
| 人員異動 | 同一關只有部分人不符 | **換人接手**：舊記錄 `holder_user` 換接任者、異動者另建一筆 |
| 無法判定 | 後台沒組織／職務，或對不到 685 選項 | 唯讀列出，請 IT 或 HR 補 |

關鍵是**人員異動不能就地改**：`role_name` 是計算欄位會跟著變，但 `role_id` 不變，
而 `walkSegment` 純用 `role_id` 走鏈（`core/03-chain-builder.js`），
上一關會無聲跳關接到新職級。換人接手則 `role_id` 完全不動，鏈零影響。

沒有接任者時才退回「停用舊記錄」，此時工具會先算出有幾個上游與幾筆 686 起點指著它，
在確認視窗紅字列出。686 的送單起點若還指著舊關卡，會在同一次操作一併改指。

**待辦**：11 與改動過的 05 都要重新上傳（685／686），並照 `docs/10` 的順序表把 `core/08` 排在 05／11 之前。
...
### kintone JS 上傳順序（⚠️ 已移到 `docs/10-工具按鈕盤點與分組.md`，以那份為準）

> 下表是舊版，2026-09-01 盤點時發現與程式碼對不起來（`tools/07` 寫死 685 卻列在 686、
> 01/02/03 沒列進 685），也還沒加入 `core/08` 與 `tools/11`。留著只為對照，**請用 `docs/10` 的表**。
...
| **人事異動對照** | **2026-09-01** | `core/08-directory` 抽出（05／11 共用）＋ `tools/11-org-sync-check`（1,098 行）＋ 30 項測試；依成因分四類，人員異動一律「換人接手」避免就地改造成跳關。順帶盤點 11 顆工具按鈕並修正上傳順序表（`docs/10`）。268 項測試全過 |
